...
int _getline(data_of_program *data)
{
	...
	static char *array_commands[10] = {0};
			...
			(array_operators[0] == '|' && errno == 0))
	{
		...
		int i = 0;
		do {
			char *temp = _strtok(i ? NULL : buff, "\n;");
			array_commands[i] = (char *) malloc(strlen(temp) + 1);
			if (array_commands[i] != NULL) {
				strcpy(array_commands[i], temp);
			}
	...
	strcpy(data->input_line, array_commands[0]);
	for (i = 0; array_commands[i]; i++)
	{
...
int check_logic_ops(char *array_commands[], int i, char array_operators[])
{
	...
	j = 0;
	while (array_commands[i] != NULL && array_commands[i][j] != '\0') {
		j++;
	}
	{
		if (strncmp(array_commands[i] + j, "&&", 2) == 0) {
		}
		{
